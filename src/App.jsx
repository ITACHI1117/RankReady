    ...
    <footer className='bg-black  flex flex-col justify-center items-center w-full'>
     <motion.div initial={{ y:200 }} whileInView={{ y:0}} transition={{ duration: 1.8,  type:"spring" }}  className='max-w-7xl w-full flex flex-col items-center justify-center gap-2 p-5 lg:p-10'>
      ...
      <article className='flex flex-col gap-1 lg:gap-3'>
        ...
        <h6 className='text-white text-center lg:text-center w-[80%]'>Be the first to get notification about our upcoming event</h6>
        <form action="">
          <div className='flex flex-row bg-white gap-3 pl-2  rounded-lg relative w-full'>
            <form action="">
          <input type="email" placeholder='Email' className='outline-none placeholder:text-[#8A8A8A] pr-1' />
          <button onClick={(e) => handleFormSubmission(e)} className='bg-[#C9FCBE] text-[#011A09] h-[100%] text-sm font-bold p-2 rounded-md font-Euclid_Light border border-white'>Submit</button>
            </form>
          </div>
        </form>
        </div>
